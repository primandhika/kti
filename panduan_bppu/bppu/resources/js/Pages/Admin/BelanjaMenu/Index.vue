<template>
  <AdminLayout
    page-title="Kelola Barang Toko"
    page-subtitle="Barang diambil dari Master Barang yang ditandai 'Tampilkan di Menu Toko'"
  >
    <div class="space-y-4">
      <!-- Filter Bar -->
      <FilterBar
        ...
        :show-download-dropdown="showDownloadDropdown"
        :kategoris="kategoris"
        ...
        @toggle-download="toggleDownloadDropdown"
      ...
      <div v-if="!groupedMenus || groupedMenus.length === 0">
        <div class="bg-white rounded-lg shadow-sm p-8 text-center">
          ...
          <h3 class="mt-2 text-sm font-medium text-gray-900">Belum ada barang toko</h3>
          <p class="mt-1 text-sm text-gray-500">Tambahkan barang dan tandai 'Tampilkan di Menu Toko'.</p>
          <div class="mt-6">
            <Link
          ...
          <div class="mb-3 pb-2 border-b-2 border-[#996600]">
            <div class="flex items-center justify-between">
              <div>
                ...
                <p class="text-xs text-gray-600">{{ group.count }} barang</p>
              </div>
            </div>
          </div>
    ...
    <EditMenuModal
      ...
      :sub-kategori-data="subKategoriData"
      :is-shop-mode="true"
    ...
    <AvailabilityModal
      ...
      @confirm="(availableOnly) => confirmDownload(availableOnly, filterWorkUnit)"
    />
  </AdminLayout>
</template>
...
import FilterBar from '@/Components/BelanjaMenu/FilterBar.vue';
...
const props = defineProps({
  ...
  subKategoriData: Array,
  subKategoris: Array,
  filters: Object,
});

// Use composables with correct baseUrl
const baseUrl = '/pengelola/belanja';
...
const handleClickOutside = (event) => {
  ...
  if (showDownloadDropdown.value) {
    const target = event.target;
    if (!target.closest('button')?.textContent?.includes('Menu') && !target.closest('.z-20')) {
