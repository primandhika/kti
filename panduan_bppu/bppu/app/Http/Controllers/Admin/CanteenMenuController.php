...
class CanteenMenuController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Get work unit for this user (assuming canteen role has work_unit_id)
        // For now, get all canteen work units or filter by user
        ...
        // Get all barangs that are marked as menu items with pagination
        ...
        $menus = $query->orderBy('sub_kategori', 'asc')
            ...
            return [
                ...
                'tampilkan_di_menu' => $barang->is_menu_item,
        ...
        // Get ALL sub categories from sub_kategori_menu table (untuk dropdown di modal edit)
        $allSubKategoris = \App\Models\SubKategoriMenu::orderBy('urutan')
            ->orderBy('nama')
            ->pluck('nama')
            ->toArray();

        // Get sub categories yang ada menu itemsnya (untuk pills filter)
        $subKategoris = \App\Models\SubKategoriMenu::orderBy('urutan')
            ->orderBy('nama')
            ->get()
        ...
        return Inertia::render('Admin/CanteenMenu/Index', [
    ...
    public function updateBarang(Request $request, $barangId)
    {
        ...
        // Update is_menu_item based on tampilkan_di_menu
        if (isset($validated['tampilkan_di_menu'])) {
            $validated['is_menu_item'] = $validated['tampilkan_di_menu'];
    ...
    /**
     * Cetak menu (HTML print) atau unduh PDF
     */
    public function downloadMenu(Request $request, CanteenMenuPdfService $pdfService)
    {
        ...
        // PDF download — pakai DomPDF (tanpa gambar)
        if (!$autoPrint) {
            return $pdfService->generateMenuPdf($theme, $workUnitId, $availableOnly);
        }

        // Print HTML — render blade langsung di browser
        $workUnits = WorkUnit::where('type', 'Kantin')->get();
        ...
        $menus = $query->get()
            ->when($availableOnly, fn($c) => $c->filter(fn($b) => $b->stok > 0))
            ->map(function($barang) {
                $display = $barang->menuDisplay;
                return [
                    'nama_barang'  => $barang->nama_barang,
                    ...
                    'gambar'       => $display->gambar ?? null,
                    'is_available' => $display->is_available ?? true,
                    'varians'      => $barang->varians->map(fn($v) => [
                        'nama'  => $v->nama_varian,
                        'harga' => $v->harga_jual,
                    ]),
                ];
        ...
        $subKategoriUrutan = \App\Models\SubKategoriMenu::orderBy('urutan')->orderBy('nama')
            ->pluck('urutan', 'nama')->toArray();

        $groupedMenus = $menus->groupBy('sub_kategori')
            ->map(fn($items) => $items->sortBy('nama_barang')->values())
            ->sortBy(fn($items, $key) => $subKategoriUrutan[$key] ?? 9999);

        return view('pdf.menu-kantin-dengan-gambar', [
            'groupedMenus'  => $groupedMenus,
